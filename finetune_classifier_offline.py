...
def init_regularizers(regularizer_config):
    ...
    if max_norm_config.get('enabled', False):
        ...
        print(
            f"      ✅ Max-Norm regularizer created (thresh={thresh}, lp_norm={lp_norm}, tau={tau})")
    else:
    ...
    if tau_norm_config.get('enabled', False):
        ...
        print(
            f"      ✅ Tau-Normalization regularizer created (lp_norm={lp_norm}, tau={tau}, frequency={tau_norm_frequency})")
    else:
            ...
            if dataset_type == 'annotation':
                ...
                if ann_train_path:
                    ...
                    current_train_dataset = AnnotationDataset(
                        ...
                        class_remapping=class_remapping_config if class_remapping_config.get(
                ...
                if not imgf_root:
                    raise ValueError(
                        f"Source {i+1}: 'imagefolder_root' is required for type 'imagefolder'.")

                imgf_train_subdir = d_cfg.get(
                    'imagefolder_train_subdir')  # Get value, could be None
                ...
                if imgf_train_subdir:  # Only proceed if subdir is not None
                    ...
                    if os.path.isdir(train_dir):
                        current_train_dataset = ImageFolderWrapper(
                            root=train_dir, transform=transform_train, class_remapping=class_remapping_config)
                        print(
                            f"     Loaded ImageFolder train from: {train_dir}")
                    else:
                        warnings.warn(
                            f"     ImageFolder train directory not found: {train_dir} (from source {i+1})")
                else:
                    print(
                        f"     Skipping ImageFolder train for source {i+1} as subdir is null.")
                ...
                if imgf_val_subdir:  # Only proceed if subdir is not None
                    ...
                    if os.path.isdir(val_dir):
                        current_val_dataset = ImageFolderWrapper(
                            root=val_dir, transform=transform_eval, class_remapping=class_remapping_config)
                        print(f"     Loaded ImageFolder val from: {val_dir}")
                    else:
                        warnings.warn(
                            f"     ImageFolder val directory not found: {val_dir} (from source {i+1})")
                else:
                    print(
                        f"     Skipping ImageFolder val for source {i+1} as subdir is null.")
                ...
                if imgf_test_subdir:  # Only proceed if subdir is not None
                    ...
                    if os.path.isdir(test_dir):
                        current_test_dataset = ImageFolderWrapper(
                            root=test_dir, transform=transform_eval, class_remapping=class_remapping_config)
                        print(f"     Loaded ImageFolder test from: {test_dir}")
                    else:
                        warnings.warn(
                            f"     ImageFolder test directory not found: {test_dir} (from source {i+1})")
                else:
                    print(
                        f"     Skipping ImageFolder test for source {i+1} as subdir is null.")
            else:
                ...
                if not first_dataset_loaded:
                    if hasattr(dataset_to_check, 'classes') and dataset_to_check.classes:
                        if final_class_names and final_class_names != dataset_to_check.classes:
                            warnings.warn(
                                f"Class names mismatch between config and dataset {i+1}. Using dataset classes.")
                        final_class_names = dataset_to_check.classes
                        print(
                            f"   First dataset loaded. Class names: {final_class_names}")
                        first_dataset_loaded = True
                    else:
                        warnings.warn(
                            f"Dataset {i+1} has no 'classes' attribute or is empty.")
                else:
                    if hasattr(dataset_to_check, 'classes') and dataset_to_check.classes != final_class_names:
                        warnings.warn(
                            f"Class name inconsistency in dataset {i+1}. Expected: {final_class_names}, Got: {dataset_to_check.classes}")
            else:
                warnings.warn(
                    f"Dataset source {i+1} yielded no valid datasets (train/val/test all None).")

            # Add to lists if datasets were created
            if current_train_dataset:
                train_datasets_list.append(current_train_dataset)
            if current_val_dataset:
                val_datasets_list.append(current_val_dataset)
            if current_test_dataset:
                test_datasets_list.append(current_test_dataset)
    ...
    try:
        ...
        for i, class_name in enumerate(class_names):
            if cm[i].sum() > 0:  # Avoid division by zero
                ...
                print(
                    f"      {class_name}: {class_accuracy:.4f} ({correct_samples}/{total_samples})")
            else:
        ...
        for i, class_name in enumerate(class_names):
            if cm[:, i].sum() > 0:  # Avoid division by zero
                ...
                print(
                    f"      {class_name}: {class_precision:.4f} ({correct_predicted}/{total_predicted})")
            else:
        ...
        if isinstance(checkpoint, dict):
            if 'model_state_dict' in checkpoint:
                ...
                if 'best_val_metric' in checkpoint:
                    print(
                        f"   Best validation metric: {checkpoint['best_val_metric']:.4f}")
    ...
    if hasattr(model, 'get_classifier') and callable(model.get_classifier):
        try:
            ...
            if isinstance(classifier_module, nn.Module):
                ...
                print(
                    f"   ⚠️ get_classifier() returned non-Module: {type(classifier_module)}")
        except Exception as e:
...
def main():
    ...
    config_file = 'tuning_classifier_config_offline.yaml'  # Use the correct config file
    ...
    try:
        ...
        # Check for pretrained model configuration first
        ...
        if pretrained_config.get('enabled', False):
            ...
            if not model_name:
                print(
                    f"❌ Error: 'model_name' must be specified in pretrained_model config")
                exit()

            # Auto-find model path if not provided
            if not model_path:
                print(f"   🔍 No model_path specified, searching for checkpoint...")
                model_path = find_pretrained_model_path(
                    model_name, config.get('results_dir', './results'))
                if not model_path:
                    print(
                        f"❌ Error: Could not find pretrained model for '{model_name}'")
                    print(
                        f"   Please specify model_path in config or ensure model exists in results directory")
                    exit()
            ...
            print(
                f"📋 Using model names from config file for classifier fine-tuning with ImageNet weights")
        ...
        if not classifier_train_config.get('enabled', False):
            print(
                "❌ Error: Classifier-only training must be enabled in config for this script.")
            print(
                "   Please set 'classifier_only_training.enabled: true' in your config.yaml")
            exit()

        dropout_rate = float(training_params.get('dropout_rate', 0.0))
        use_amp = classifier_train_config.get(
            'use_amp', training_params.get('use_amp', True))
        train_ratio = float(training_params.get('train_ratio', 1.0))
        if not (0.0 < train_ratio <= 1.0):
            warnings.warn(
                f"⚠️ Invalid train_ratio ({train_ratio}). Clamping to 1.0.")
            train_ratio = 1.0
        ...
        print(
            f"❌ Error: Invalid value type or missing key in configuration file: {e}")
        print("   Please check the data types and structure in config.yaml.")
        exit()

    # --- Device Setup ---
    device, gpu_count = get_device(device_config.get(
        'use_cuda', True), device_config.get('multi_gpu', True))
    ...
    for model_name in model_names:
        ...
        model_results_dir = os.path.join(
            config['results_dir'], f"{model_name}_classifier_finetune")
        ...
        while current_batch_size >= 1:
            ...
            # --- Initialize Model (ImageNet pretrained or from checkpoint) ---
            try:
                # Use config class count or default
                temp_num_classes = num_classes if num_classes is not None else 5

                if use_pretrained_checkpoint:
                    # Load from checkpoint
                    print(
                        f"📂 Loading model '{model_name}' from checkpoint: {pretrained_model_path}")
                    ...
                    print(
                        f"📂 Initializing model '{model_name}' with ImageNet pretrained weights for classifier fine-tuning...")
                    model, input_size, model_specific_transform, model_config = initialize_model(
                        model_name,
                ...
                if aug_config.get('enabled', False):
                    print(
                        f"   🎨 Using ADVANCED augmentation strategy: {aug_config.get('strategy', 'light')}")
                    aug_strategy = create_augmentation_strategy(
                        config, model_config)
                    ...
                    transform_train = transforms.Compose([
                        ...
                        transforms.Normalize(
                            mean=model_config['mean'], std=model_config['std'])
                    ])
                ...
                print(
                    f"   Input size: {model_config.get('input_size', 224)}x{model_config.get('input_size', 224)}")
                print(
                    f"   Augmentation enabled: {aug_config.get('enabled', False)}")
                ...
                if not train_datasets_list:
                    raise ValueError(
                        "❌ No training datasets were loaded successfully.")
                if not val_datasets_list:
                    warnings.warn(
                        "⚠️ No validation datasets were loaded. Validation phase will be skipped.")
                if not test_datasets_list:
                    warnings.warn(
                        "⚠️ No test datasets were loaded. Final evaluation will be skipped.")

                # Combine datasets if multiple
                if len(train_datasets_list) > 1:
                    print(
                        f"   Combining {len(train_datasets_list)} training datasets...")
                    final_train_dataset_full = CombinedDataset(
                        train_datasets_list)
                else:
                ...
                if train_ratio < 1.0:
                    subset_size = math.ceil(
                        len(final_train_dataset_full) * train_ratio)
                    indices = list(range(subset_size))
                    final_train_dataset = Subset(
                        final_train_dataset_full, indices)
                ...
                if final_class_names is None and hasattr(final_train_dataset, 'classes'):
                    ...
                    print(
                        f"   Inferred {num_classes} classes from dataset: {final_class_names}")
                ...
                if hasattr(model, 'get_classifier') and callable(model.get_classifier):
                    try:
                        ...
                        if isinstance(classifier_module, nn.Module):
                            ...
                            print(
                                f"   ✅ Unfroze classifier module: {type(classifier_module).__name__}")
                        else:
                            print(
                                f"   ⚠️ get_classifier() returned non-Module: {type(classifier_module)}")
                    except Exception as e:
                        warnings.warn(
                            f"   ⚠️ Error calling model.get_classifier(): {e}")
                else:
                    # Manual approach for common architectures
                    print(
                        "   ⚠️ model.get_classifier() method not found. Trying manual layer unfreezing...")
                    if hasattr(model, 'classifier'):
                        ...
                        warnings.warn(
                            "   ⚠️ Could not find classifier layer to unfreeze")
                ...
                if not classifier_params:
                    print(
                        f"❌ No trainable parameters found for classifier-only training. Skipping model '{model_name}'.")
                    break
                ...
                cls_sampler_config = classifier_train_config.get(
                    'weighted_random_sampler', {})
                effective_sampler_config = get_effective_sampler_config(
                    main_sampler_config, cls_sampler_config)
                ...
                if effective_sampler_config.get('enabled', False):
                    try:
                        ...
                        print(
                            f"   ✅ WeightedRandomSampler created for classifier training")
                        print(
                            f"      Strategy: {effective_sampler_config.get('weight_calculation', 'inverse')}")
                        ...
                        print(
                            f"   ⚠️ Failed to create WeightedRandomSampler: {e}")
                ...
                cls_optimizer_config = classifier_train_config.get(
                    'optimizer', {'type': 'Adam', 'params': {'lr': 1e-4}})
                cls_optimizer_type = cls_optimizer_config.get(
                    'type', 'Adam').lower()
                ...
                if cls_optimizer_type == 'adam':
                    optimizer = optim.Adam(
                        classifier_params, **cls_optimizer_params)
                    ...
                    warnings.warn(
                        f"Unsupported optimizer: {cls_optimizer_type}. Using Adam.")
                ...
                print(
                    f"   Learning rate: {cls_optimizer_params.get('lr', 'default')}")
                print(
                    f"   Weight decay: {cls_optimizer_params.get('weight_decay', 'default')}")

                # --- Setup Scheduler ---
                cls_scheduler_config = classifier_train_config.get(
                    'scheduler', {'type': 'ReduceLROnPlateau'})
                cls_scheduler_type = cls_scheduler_config.get(
                    'type', 'ReduceLROnPlateau').lower()
                ...
                if hasattr(final_train_dataset_full, 'targets') and hasattr(final_train_dataset_full, 'classes'):
                    ...
                    cls_num_list = [int(np.sum(targets_np == i)) for i in range(
                        len(final_train_dataset_full.classes))]
                else:
                ...
                cls_criterion_b_name = classifier_train_config.get(
                    'criterion_b', '').lower()
                cls_criterion_b_params = classifier_train_config.get(
                    'criterion_b_params', {})
                cls_first_stage_epochs = classifier_train_config.get(
                ...
                criterion_a = get_criterion(
                    cls_criterion_a_name, num_classes, device, cls_criterion_a_params)
                criterion_b = None
                if cls_first_stage_epochs > 0 and cls_criterion_b_name:
                    criterion_b = get_criterion(
                        cls_criterion_b_name, num_classes, device, cls_criterion_b_params)

                # Add noise_sigma parameters to optimizer if needed
                if cls_criterion_a_name in ['bmcloss', 'gailoss']:
                    optimizer.add_param_group(
                        {'params': criterion_a.noise_sigma, 'lr': 1e-2, 'name': 'noise_sigma_a'})
                if criterion_b and cls_criterion_b_name in ['bmcloss', 'gailoss']:
                    optimizer.add_param_group(
                        {'params': criterion_b.noise_sigma, 'lr': 1e-2, 'name': 'noise_sigma_b'})

                print(f"\n📉 Classifier Criterion: {cls_criterion_a_name}")
                if criterion_b:
                    print(
                        f"   Dual criterion training: {cls_first_stage_epochs} epochs with {cls_criterion_a_name}, then {cls_criterion_b_name}")

                # --- Initialize Regularizers ---
                print(f"\n🔧 Initializing regularizers for classifier training...")
                cls_regularizer_config = classifier_train_config.get(
                    'regularization', {})
                max_norm_reg, tau_norm_reg, tau_freq = init_regularizers(
                    cls_regularizer_config)

                # --- Training Configuration ---
                num_epochs = classifier_train_config.get('num_epochs', 10)
                patience = classifier_train_config.get('patience', 3)
                clip_grad_norm = classifier_train_config.get(
                    'clip_grad_norm', 1.0)
                cls_train_ratio = classifier_train_config.get(
                    'train_ratio', train_ratio)

                model_save_path = os.path.join(
                    model_results_dir, f'{model_name}_classifier_best.pth')
                log_save_path = os.path.join(
                    model_results_dir, f'{model_name}_classifier_training_log.csv')

                print(
                    f"\n🚀 Starting classifier-only fine-tuning for '{model_name}'...")
                print(f"   Epochs: {num_epochs}, Patience: {patience}")
                print(
                    f"   Batch size: {current_batch_size}, Train ratio: {cls_train_ratio}")
                ...
                print(
                    f"✅ Classifier fine-tuning completed successfully for '{model_name}' with batch size {current_batch_size}.")
                break
                ...
                if "CUDA out of memory" in str(e):
                    ...
                    if 'optimizer' in locals():
                        del optimizer
                    if 'criterion_a' in locals():
                        del criterion_a
                    if 'val_loader' in locals() and val_loader:
                        del val_loader
                    if 'test_loader' in locals() and test_loader:
                        del test_loader
                    if 'scheduler' in locals() and scheduler:
                        del scheduler
                    if 'criterion_b' in locals() and criterion_b:
                        del criterion_b
                    ...
                    if current_batch_size < 1:
                        print(
                            f"❌ Batch size reduced below 1. Cannot train. Skipping model '{model_name}'.")
                        break
                else:
        ...
        if model_trained_successfully:
            ...
            plot_file_path = os.path.join(
                model_results_dir, f'{model_name}_classifier_training_curves.png')
            if history:
                try:
                    plot_training_curves(
                        history,
            ...
            if test_loader:
                ...
                model.eval()
                report_base_path = os.path.join(
                    model_results_dir, f'{model_name}_classifier_test_eval')
                ...
                try:
                    ...
                    if y_true and y_pred:
                        print(
                            f"   📊 Generating comprehensive evaluation report with confusion matrices...")
                        print(f"   📈 Evaluation will include:")
                        print(
                            f"      • Classification report with precision, recall, F1-score")
                        print(f"      • Raw counts confusion matrix")
                        print(
                            f"      • Normalized confusion matrix (by true labels - recall)")
                        print(
                            f"      • Normalized confusion matrix (by predicted labels - precision)")
                        print(
                            f"   💾 All visualizations will be saved to: {report_base_path}_*.png")

                        # Display quick stats in console first
                        display_confusion_matrix_stats(
                            y_true, y_pred, final_class_names)

                        # Generate full report with visualizations
                        report_classification(
                            y_true, y_pred, final_class_names, save_path_base=report_base_path)

                        print(
                            f"   ✅ Evaluation completed! Check the following files:")
                        print(
                            f"      📄 Text report: {report_base_path}_report.txt")
                        print(
                            f"      📊 Raw confusion matrix: {report_base_path}_cm_raw.png")
                        print(
                            f"      📈 Recall confusion matrix: {report_base_path}_cm_norm_true.png")
                        print(
                            f"      📉 Precision confusion matrix: {report_base_path}_cm_norm_pred.png")
                    else:
                        print(
                            "   ⚠️ Skipping evaluation report due to inference issues.")
            ...
            if test_loader and final_test_dataset:
                ...
                gradcam_save_dir = os.path.join(
                    model_results_dir, "gradcam_visualizations")
        ...
        if 'model' in locals():
            del model
        if 'train_datasets_list' in locals():
            del train_datasets_list
        if 'val_datasets_list' in locals():
            del val_datasets_list
        if 'test_datasets_list' in locals():
            del test_datasets_list
        gc.collect()
        if device.type == 'cuda':
            torch.cuda.empty_cache()
