...
class SimpleInference:
    ...
    def load_dataset(self, split='test', custom_annotation=None, 
                     custom_root=None,
                     imgf_root= None # Khanh November 1st
                     ):
        ...
        if dataset_config.get('type', 'annotation').lower() == 'annotation':
            ...
            if imgf_root is None:
                imgf_root = dataset_config.get('imagefolder_root')
            # ---------------------------
            # imgf_root: input to the function instead of from config
            # ---------------------------
    ...
    def run_inference(self, dataset, batch_size=32, save_scores=False, scores_output_path=None):
        ...
        # Khanh implementation: Sorting predictions by image names
        # ------------------------
        predictions = np.array(all_predictions)
        confidences = np.array(all_confidences)
        image_names = all_sample_paths
        gt_labels = np.array(all_true_labels)

        # Function to extract the numeric part of the filename
        def extract_num(path):
            return int(re.search(r'(\d+)\.jpg', path).group(1))

        # Sort all three arrays by numeric order of filename
        sorted_data = sorted(zip(image_names, predictions, confidences, gt_labels),
                            key=lambda x: extract_num(x[0]))

        # Unzip back
        sorted_image_names, sorted_predictions, sorted_confidences, gt_labels = map(np.array, zip(*sorted_data))
        ...
        return {
            ...
            'probabilities': np.array(all_probabilities),
            'sample_paths': sorted_image_names,
            'accuracy': accuracy,
            'total_samples': total_samples,
            'out_of_range_samples': out_of_range_samples if hasattr(self, 'class_mismatch') and self.class_mismatch else 0,
            'scores_saved': save_scores,
...
def run_simple_inference(
    ...
    try:
        ...
        # print("-----------------------\nOutput of run_inference_simple() function!")
        # print(results['predictions'])
        # print(results['true_labels'])
        # print(results['confidences'])
        # print(results['sample_paths'])
