...
def plot_confusion_matrix_manual(y_true, y_pred, class_names, title='Confusion Matrix', 
    ...
    max_chars_per_line = max(40, len(class_names) * 8)  # Ít nhất 40 ký tự, tăng theo số lớp
    ...
    if normalize:
        cbar.ax.set_ylabel('Tỷ lệ phần trăm', rotation=-90, va="bottom", fontsize=12, labelpad=20)
    else:
        cbar.ax.set_ylabel('Số lượng', rotation=-90, va="bottom", fontsize=12, labelpad=20)
    ...
    title_pad = max(20, num_title_lines * 15)  # Tăng padding cho title nhiều dòng
...
def calculate_classification_metrics_v2(file_path):
    ...
    save_dir_norm = file_path.replace('.txt', '_confusion_matrix_normalized.png')
    plot_confusion_matrix_manual(
        true_labels, predicted_labels, unique_labels_sorted,
        title='Confusion Matrix - WD + MaxNorm + Class Balanced Softmax Finetune',
        normalize=True,
