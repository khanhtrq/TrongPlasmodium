...
try:
    ...
    warnings.warn("⚠️ timm library not available. Some advanced augmentations will not work.")
...
class TimmAugmentationStrategy:
    ...
    def __init__(self, 
        ...
        if not TIMM_AVAILABLE and strategy in ['heavy', 'extreme']:
            warnings.warn("⚠️ Advanced augmentation strategies require timm. Falling back to 'medium' strategy.")
    ...
    def compare_strategies(self, strategies=None):
        ...
        for strategy in strategies:
            ...
            try:
                ...
                # Count transforms
                train_count = len(train_transform.transforms) if hasattr(train_transform, 'transforms') else 1
                eval_count = len(eval_transform.transforms) if hasattr(eval_transform, 'transforms') else 1
                ...
                if hasattr(train_transform, 'transforms'):
                    key_transforms = [t.__class__.__name__ for t in train_transform.transforms]
                    print(f"   Key training ops: {', '.join(key_transforms[:5])}")
                    if len(key_transforms) > 5:
                        print(f"                     ... and {len(key_transforms) - 5} more")
        ...
        if hasattr(eval_transform, 'transforms'):
            eval_ops = [t.__class__.__name__ for t in eval_transform.transforms]
    ...
    def _get_heavy_transform(self):
        ...
        if not TIMM_AVAILABLE:
            warnings.warn("⚠️ timm not available, falling back to medium augmentation")
            return self._get_medium_transform()
        ...
        try:
            auto_augment = auto_augment_transform(
                ...
                hparams={'translate_const': int(0.45 * min(self.input_size)), 'img_mean': tuple([min(255, round(255 * x)) for x in self.mean])}
            )
    ...
    def _get_extreme_transform(self):
        ...
        try:            # Use timm's create_transform with aggressive settings
            ...
            warnings.warn(f"⚠️ Could not create extreme transform: {e}, falling back to heavy")
            return self._get_heavy_transform()
    ...
    def _get_interpolation(self):
        ...
        return interpolation_map.get(self.interpolation, Image.BILINEAR)    
...
class RandAugmentTransform:
    ...
    def _solarize_add(self, img):
        ...
        img_array = np.where(img_array < threshold, img_array + threshold, img_array)
...
def get_timm_transform(model_name, input_size=224, is_training=True, **kwargs):
    ...
    try:
        ...
        warnings.warn(f"⚠️ Could not create timm transform for {model_name}: {e}")
        return None
...
def test_image_augmentation(image_path, output_dir="augmentation_test_results"):
    ...
    fig.suptitle(f'Augmentation Test Results - {os.path.basename(image_path)}', fontsize=16)
    ...
    for idx, strategy in enumerate(strategies):
        ...
        try:
            ...
            if isinstance(augmented_tensor, torch.Tensor):
                ...
                augmented_img = Image.fromarray((img_array * 255).astype(np.uint8))
            else:
            ...
            axes[row, col].set_title(f'{strategy.capitalize()} Strategy (Error)')
            axes[row, col].axis('off')
...
def test_multiple_augmentations(image_path, strategy='medium', num_samples=9, output_dir="multiple_augmentations"):
    ...
    fig.suptitle(f'Multiple {strategy.capitalize()} Augmentations - {os.path.basename(image_path)}', fontsize=16)
    ...
    for i in range(num_samples):
        try:
            ...
            # Save augmented image
            save_path = os.path.join(output_dir, f'{strategy}_sample_{i+1:02d}.jpg')
            augmented_img.save(save_path)
    ...
    plt.tight_layout()
    comparison_path = os.path.join(output_dir, f'multiple_{strategy}_augmentations.png')
    plt.savefig(comparison_path, dpi=150, bbox_inches='tight')
...
def interactive_augmentation_test():
    ...
    if choice in ['2', '3']:
        ...
        while True:
            try:
                num_samples = int(input("Enter number of samples (default 9): ").strip() or "9")
                if num_samples > 0:
                    break
                else:
...
def test_transforms_only():
    ...
    print(f"   • Normalization: mean={aug_strategy.mean}, std={aug_strategy.std}")
    ...
    if hasattr(eval_transform, 'transforms'):
        for i, transform in enumerate(eval_transform.transforms, 1):
            ...
            if transform_name == 'Resize':
                ...
                print(f"   🎯 Impact: Changes data type from uint8 [0,255] to float32 [0,1]")
                print(f"   ⚙️  Method: Divides pixel values by 255, changes HWC to CHW format")
                
            elif transform_name == 'Normalize':
                mean = getattr(transform, 'mean', 'Unknown')
                std = getattr(transform, 'std', 'Unknown')
                print(f"   📐 Purpose: Normalize pixel values using ImageNet statistics")
                print(f"   🎯 Impact: Centers data around 0, reduces training instability")
